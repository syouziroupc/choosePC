...
const version = argument("--version", "knowledge-2026-08-13.1");
...
const cpus = await readJson("knowledge/hardware/cpu/catalog.json");
const gpus = await readJson("knowledge/hardware/gpu/catalog.json");
...
for (const source of sources) {
  lines.push(`INSERT INTO source_documents (id, url, publisher, title, retrieved_at, license_note, content_hash) VALUES (${quote(source.id)}, ${quote(source.url)}, ${quote(source.publisher)}, ${quote(source.title ?? null)}, ${quote(source.retrievedAt)}, NULL, NULL) ON CONFLICT(id) DO UPDATE SET url = excluded.url, publisher = excluded.publisher, title = excluded.title, retrieved_at = excluded.retrieved_at;`);
}
