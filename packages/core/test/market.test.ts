...
describe("robust market estimator", () => {
  it("returns a broad low-confidence range from one valid observation", () => { const r=estimateMarket([observation(30000)],{now}); expect(r.estimate).not.toBeNull(); expect(r.estimate!.sampleCount).toBe(1); expect(r.estimate!.effectiveSampleCount).toBeGreaterThan(0); expect(r.estimate!.confidence).toBeLessThanOrEqual(28); expect(r.estimate!.dataQuality).toBe("sparse"); expect(r.estimate!.method).toBe("weighted_median_mad_v1"); expect(r.estimate!.lowPriceJpy).toBe(21000); expect(r.estimate!.highPriceJpy).toBe(39000); });
