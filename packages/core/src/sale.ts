...
export interface SaleAssessment {
  ...
  route: SaleDecision;
  confidence: number;
  market?: { fairPriceJpy: number; lowPriceJpy?: number; highPriceJpy?: number; sampleCount: number; confidence: number };
  reasons: string[];
}

function result(decision: SaleDecision, confidence: number, reasons: string[], market?: SaleAssessment["market"]): SaleAssessment {
  return { decision, route: decision, confidence, market, reasons };
}
...
export function assessSale(pc: NormalizedPC, market?: MarketEstimate | null): SaleAssessment {
  if (!market || market.fairPriceJpy <= 0 || market.source === "user_estimate" || market.sampleCount < 3) {
    return result("insufficient_data", 0, ["売却相場の根拠データが不足しています。"]);
  }
  const confidence = marketConfidence(market);
  const resultMarket = {
    fairPriceJpy: market.fairPriceJpy,
    lowPriceJpy: market.lowPriceJpy,
    highPriceJpy: market.highPriceJpy,
