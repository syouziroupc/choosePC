...
const PROFILES: MerchantProfile[] = [
  ...
  {
    merchant: "Yahoo!オークション",
    parserName: "yahoo-auctions-jp",
    hosts: ["auctions.yahoo.co.jp"],
    titleClasses: ["ProductTitle", "ProductTitle__text", "ItemTitle"],
    priceClasses: ["Price", "Price__value"],
    scopeClasses: ["ProductExplanation", "ProductDetail", "Description"],
  },
  {
    merchant: "メルカリ",
...
function inferStockState(text: string): { state: ParsedStockState; confidence: number } {
  ...
  if (/売り切れ|売切れ|sold\s*out|売却済み|このオークションは終了|オークションは終了|落札価格/.test(normalized)) return { state: "sold", confidence: 92 };
